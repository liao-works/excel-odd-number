...
class DPDProcessor:
  ...
  def process_dpd_data(self, original_file_data: pd.DataFrame, original_detail_file_data: pd.DataFrame, template_path: str, output_path: str):
    ...
    try:
        ...
        if sub_order_sheet is not None:
            self.process_sub_order_sheet(template_workbook, sub_order_sheet, original_detail_file_data, first_empty_row, collection_total_row, original_file_data_count)
  ...
  def process_sub_order_sheet(self, template_workbook: Workbook, sub_order_sheet: Worksheet, original_detail_file_data: pd.DataFrame, first_empty_row: int, collection_total_row: int, original_file_data_count: int):
    """
    处理子单号工作表
    """
    ...
    try:
        self.fill_sub_order_sheet(sub_order_sheet, original_detail_file_data, first_empty_row)
        self.logger.info("子单号工作表处理完成")
    except Exception as e:
        self.logger.error(f"处理子单号工作表时出错: {str(e)}")
        raise

  def fill_sub_order_sheet(self, sub_order_sheet: Worksheet, original_detail_file_data: pd.DataFrame, first_empty_row: int):
    """
    填充子单号工作表
    """
    try:
        ...
        # 检查原始数据的列名和可用数据
        available_columns = original_detail_file_data.columns.tolist()
        self.logger.info(f"原始明细数据可用列: {available_columns}")
        self.logger.info(f"模板表头到列号映射: {header_column_mapping}")
        ...
        for data_row_idx in range(data_row_count):
            target_row = first_empty_row + data_row_idx
            row_data = original_detail_file_data.iloc[data_row_idx]
